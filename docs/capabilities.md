# What this watch face _could_ use — features and data sources
...
| **proven**    | read off _this_ Pixel Watch 4 at least once — the value is trustworthy |
...
that has never existed — shipped and did nothing. Anything moved from _available_ to _in
use_ has to be seen on the wrist.
...
| `MONTH`                                                                                    | **in use** — with `DAY`, gates all eight calendar states. ⚠ **1 = January**, assumed and not yet read off the watch; a zero-based `MONTH` would put every celebration a month early              |
| `MONTH_Z`, `MONTH_F`, `MONTH_S`                                                            | available — padded, full name, short name                                                                                                                                                        |
...
| source                                                                                             | note                                                                                                                                 |
| -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `IS_24_HOUR_MODE`                                                                                  | available — the `<DigitalClock>` already follows the system setting without reading this; you only need it to _branch_ on the format |
| `IS_DAYLIGHT_SAVING_TIME`                                                                          | available                                                                                                                            |
| `TIMEZONE`, `TIMEZONE_ABB`, `TIMEZONE_ID`                                                          | available — strings                                                                                                                  |
| `TIMEZONE_OFFSET`, `TIMEZONE_OFFSET_DST`, `TIMEZONE_OFFSET_MINUTES`, `TIMEZONE_OFFSET_MINUTES_DST` | available — numeric, so these _can_ be compared in a `Condition`                                                                     |
...
| `BATTERY_IS_LOW`                                                | **in use, proven** — fires around 6–8%; drives the coral text and gauge. Note Wear OS paints its _own_ low-battery mark near the bottom edge, which is not part of the face   |
...
⚠ **`<Gyro>` can read _only_ these eight.** Its `gyroArithmeticExpressionType` unions
...
| `STEP_GOAL`    | **proven** — the wearer's _real_ goal, measured 10000. No need to hardcode                                                                                       |
...
| source                      | note                                                                                                                               |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `UNREAD_NOTIFICATION_COUNT` | available — untouched. The one genuinely unused _system_ signal with obvious character potential (the blob noticing you have mail) |
...
is not involved, and it is _not_ a permission issue (`RECEIVE_WEATHER` is
...
| `WEATHER.UV_INDEX`                                    | **in use** — drives the sunglasses at `>= 3` (the bottom of WHO/EPA "moderate"). Integer, 0–11+. ⚠ **the branch is proven, the provider is not**: the capture sweep mocks it to a literal 8, and no live reading has ever been seen                   |
...
## 1.13 Three source namespaces that are _not_ in `sourceType.xsd`
...
- **Sunrise / sunset time** — note `SUNRISE_SUNSET` _is_ a complication provider policy, so
...
- **Calendar events, alarms, music, messages** — as _sources_. All four are reachable as
...
| `Fill`, `Stroke`                                    | mostly solid; caps `BUTT`/`ROUND`/`SQUARE`, joins `MITER`/`ROUND`/`BEVEL`. ⚠ one `RadialGradient` now, on the moon's disc — **in use, unproven**: checked against the SVG preview and the differ, not yet seen on the wrist |
...
| `Gyro`                                              | 11 — ⚠ each accessory that is a _sibling_ of a blob group repeats its blob's gain by hand. Changing a gain means changing every one                                                                                         |
...
| `Launch`                                            | 2 — `date_interactive` → `CALENDAR`, `chip_heart_rate` → `HEALTH_HEART_RATE`. ⚠ **schema-valid, never tapped on the wrist.** `@target` is a union with `xs:string`, so a typo validates                                     |
...
| **`LinearGradient` / `SweepGradient`**                             | `colors` + `positions`; sweep also takes a direction. `RadialGradient`, their sibling, is now in use on the moon (§2.1) - the same volume-without-a-bitmap trick, unclaimed                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **`HsbFilter`**                                                    | `hueRotate` / `saturate` / `brightness` on an image. Would let _one_ bitmap serve all seven weekday hues                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
and an unimplemented function inside a `Transform` fails _silently_ while passing the
validator.
...
5. **`ComplicationSlot`** — the only door to calendar, sunrise/sunset, contacts and
   third-party data, and the only tappable element that can open an app `Launch` has no
   target for. ⚠ **Scene-level only, max 8**: `groupElement.xsd` has no `ComplicationSlot`,
   so a slot cannot sit inside a `Group`, a `Condition` or a `Gyro` — no conditional
   visibility and no parallax. It takes `Variant`, so hiding it in ambient still works.
