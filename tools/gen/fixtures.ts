 ...
 */
export const weekdayStates = (): Record<string, StateDelta> => {
 ...
 */
export const STATES: Record<string, StateDelta> = {
  ambient: {},
  baseline: {},
...
export const valuesFor = (
  ...
  if (delta === undefined) {
    throw new Error(`unknown state "${state}". One of: ${Object.keys(STATES).join(', ')}`)
  }
  const out: Record<string, number> = { ...BASE }
  for (const [k, v] of Object.entries(delta)) {
    // A StateDelta also carries `time` and `weekday`, which are display strings
    // rather than sources. Only the numbers belong in a value set.
    if (typeof v === 'number') out[k] = v
  }
  return { ...out, ...overrides } as Record<NumericSource, number>
}
...
export const EVAL_GRID = (): Array<Record<NumericSource, number>> => {
  const rows = Object.keys(STATES).map((s) => valuesFor(s))
  const keys = Object.keys(CANDIDATES) as NumericSource[]

  // 2. One factor at a time.
  for (const k of keys) {
    if (k === 'SECOND' || k === 'SECOND_MILLISECOND') continue
    for (const v of CANDIDATES[k]) rows.push(valuesFor('baseline', { [k]: v }))
  }
  ...
  for (let i = 0; i < 600; i++) {
    ...
    for (const k of keys) row[k] = pick(CANDIDATES[k])
