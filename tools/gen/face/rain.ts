 ...
 */
const drop = (item: Drop, index: number): Node => {
	const label = String(index + 1).padStart(2, '0');
	const widthExtra = widthGrowth(item.w0);
	const renderedWidth = item.w0 + widthExtra;
	const renderedHeight = item.h0 + item.hx;
	// The Part box is the shape's bounding box with a pixel of slack, so a drop
	// at full width is not clipped by its own parent.
	const boxW = Math.ceil(renderedWidth) + 1;
	const boxH = Math.ceil(renderedHeight) + 1;
	...
	const dropPhase = phase(item.hz, item.ph);

	return el('Group', { ...G.CANVAS, name: `rain_drop_${label}`, alpha: 255 }, [
		el('Transform', {
			target: 'y',
			value: `(${grow(item.fall, item.fallX, PRECIP)}) * ${dropPhase}`
		}),
		el('Transform', {
			target: 'alpha',
			value: `${triangleAlpha(dropPhase)} * ${precipGate(item.gate)}`
		}),
		el(
			'PartDraw',
			{ name: `rain_shape_${label}`, x: item.x, y: item.y, width: boxW, height: boxH },
			[
				el(
					'RoundRectangle',
					{
						...
						cornerRadiusX: renderedWidth / 2,
						cornerRadiusY: renderedWidth / 2
					},
					[
						el('Transform', { target: 'width', value: grow(item.w0, widthExtra, PRECIP) }),
						el('Transform', { target: 'height', value: grow(item.h0, item.hx, PRECIP) }),
